...
def my_parser(request):
    return render(request, 'myparser/parser.html')
...
def get_pages_count(html):
    soup = BS(html, 'lxml')
    pagination = soup.find_all('button', class_='_1ItKI _1KouS _1wUb0') + soup.find_all('button', class_='_1ItKI _1wUb0')
    if pagination:
...
def get_content(html):
    soup = BS(html, 'lxml')
    #print(soup)
    items = soup.find_all('div', class_='_3AkUH templates-object-card')

    #print(items)
    realty = []
    for item in items:
        realty.append({'adress': item.find('div', class_='_3d4Ao').get_text(strip=True).replace('\n   ', ','),
                       'price': item.find('span', class_='_1s0Jo _2Ylcq').get_text(strip=True).replace('руб.', ''),
...
def message(request):
    ...
    os.startfile(FILE)
    return render(request, 'myparser/message.html', {'message': pages_count,'parser': len(realty)})
